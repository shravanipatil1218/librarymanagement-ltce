...
export function BookDetailDialog({ book, open, onOpenChange }: BookDetailDialogProps) {
  ...
  const handleIssue = (e: React.FormEvent) => {
    ...
    toast({
      title: "Book Issued!",
      description: `"${book.title}" issued for ${duration} days.`,
    });
        ...
        {!showIssueForm ? (
          ...
          <form onSubmit={handleIssue} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input type="number" min="7" max="30" value={duration} onChange={(e) => setDuration(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label>Return Deadline</Label>
              <Input type="date" value={new Date(Date.now() + parseInt(duration) * 86400000).toISOString().split("T")[0]} readOnly />
            </div>
